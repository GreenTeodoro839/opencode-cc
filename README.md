...
- **Codex CLI 兼容。** 提供 `POST /v1/responses`，将 Codex 使用的 Responses API 请求、流式文本事件和函数调用双向转换为 Zen `/go/v1/chat/completions`，支持 Codex 的多轮工具调用。
...
`model` 可以直接填写 Zen 模型 ID，也可以填写控制面板中配置的映射别名。推荐编码模型 `kimi-k2.7-code`。
...
Codex 请求 `/v1/responses` 时，代理将 `instructions`、`input[]`、`function_call`、`function_call_output` 和 function tools 转成 OpenAI Chat Completions 消息。Zen 返回的文本增量、`tool_calls` 参数分片和 usage 会重新组装成 `response.output_text.delta`、`response.function_call_arguments.delta`、`response.completed` 等标准 Responses SSE 事件。
...
| POST | `/v1/responses` | OpenAI Responses API，兼容 Codex CLI，支持流式文本和函数调用 |
